...
ChatGPT Codex login, model catalog, usage, and optional search/image capabilities. This release targets official DeepSeek Harness `0.1.7-alpha.2` and contains built Host/Client files only; it has no sibling-repository source, workstation path, link:, or workspace: dependency.
