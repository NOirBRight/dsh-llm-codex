window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region node_modules/.pnpm/dsh-llm-providers-ui@https+++github.com+NOirBRight+dsh-llm-providers-ui+releases+downlo_9d4abfc931bf3221fbaeed9b05aa2567/node_modules/dsh-llm-providers-ui/lib/usage-readers.js
		...
		*/
		...
		function decodeCodexAuthStatus$1(value) {
			...
			for (const rateLimitValue of usage.rateLimits) {
				const rateLimit = recordUsageValue$1(rateLimitValue);
				if (rateLimit === void 0 || !nonEmptyString$1(rateLimit.id) || !Array.isArray(rateLimit.windows)) return {
					status: "error",
					message: "malformed usage response"
				};
				if (rateLimit.name !== void 0 && !nonEmptyString$1(rateLimit.name)) return {
					status: "error",
					message: "malformed usage response"
				};
				for (const [index, windowValue] of rateLimit.windows.entries()) {
					const quotaWindow = recordUsageValue$1(windowValue);
					if (quotaWindow === void 0 || !nonNegativeNumber$1(quotaWindow.remainingPercent) || quotaWindow.remainingPercent > 100 || !nonNegativeNumber$1(quotaWindow.windowSeconds) || quotaWindow.windowSeconds === 0) return {
						status: "error",
						message: "malformed usage response"
					};
					if (quotaWindow.resetsAt !== void 0 && !nonEmptyString$1(quotaWindow.resetsAt)) return {
						status: "error",
						message: "malformed usage response"
					};
					...
					windows.push({
						...
						label,
						shortLabel: shortLabel$1(duration),
		...
		async function readCodexUsage(rpc, signal) {
			...
			while (!signal.aborted) {
				...
				last = result.ok ? decodeCodexAuthStatus$1(result.value) : credentialFailure(result.error) ? { status: "logged-out" } : {
					status: "error",
		...
		*/
		...
		//#endregion
		//#region node_modules/.pnpm/dsh-llm-providers-ui@https+++github.com+NOirBRight+dsh-llm-providers-ui+releases+downlo_9d4abfc931bf3221fbaeed9b05aa2567/node_modules/dsh-llm-providers-ui/lib/provider-ui.js
		/** Plain-object guard shared by the reader factories and the sidebar cache validator. */
		...
		*/
		...
		//#endregion
		//#region node_modules/.pnpm/dsh-llm-providers-ui@https+++github.com+NOirBRight+dsh-llm-providers-ui+releases+downlo_9d4abfc931bf3221fbaeed9b05aa2567/node_modules/dsh-llm-providers-ui/lib/sortable.js
		...
		*/
		...
		const USAGE_SETTLE_START_MS = 1500;
		...
		function modelSettingsOf(draft) {
			...
			return {
				...
				...draft.efforts === void 0 ? {} : { efforts: draft.efforts },
		...
		function headerQuotaOf(auth, lastUsage, t) {
			const first = (auth.status === "signed-in" ? auth.usage : lastUsage)?.rateLimits[0];
			const window = first?.windows[0];
			if (first === void 0 || window === void 0) return null;
			const remaining = window.remainingPercent;
			if (!Number.isFinite(remaining) || remaining < 0 || remaining > 100) return null;
			const label = windowLabel(window.windowSeconds, t);
			const displayLabel = first.name === void 0 || first.windows.length === 1 ? first.name ?? label : first.name + " · " + label;
		...
		function CodexPluginCard(props) {
			...
			const settings = (0, react.useMemo)(() => decodeCodexSettings(snapshot.value), [snapshot.value]);
			const [open, setOpen] = (0, react.useState)(false);
			const initial = (0, react.useMemo)(() => settings?.models.map(modelDraftOf), [settings]);
			const [source, setSource] = (0, react.useState)(initial);
			const [draft, setDraft] = (0, react.useState)(initial);
			const [capabilities, setCapabilities] = (0, react.useState)(settings === void 0 ? void 0 : capabilityOf(settings));
			...
			const [authAnswered, setAuthAnswered] = (0, react.useState)(false);
			...
			const dirtyCaps = settings !== void 0 && capabilities !== void 0 && JSON.stringify(capabilityOf(settings)) !== JSON.stringify(capabilities);
			...
			(0, react.useEffect)(() => {
				if (snapshot.status !== "ready" || settings === void 0) return;
			...
			const applyAuthStatus = (0, react.useCallback)((next) => {
			...
			const usageSettled = refreshError === void 0 && liveQuota !== null;
			/**
			...
			*/
			...
			(0, react.useEffect)(() => {
				...
				const interval = auth.status === "signing-in" ? 1e3 : auth.status === "signed-in" && usageSettled ? USAGE_POLL_INTERVAL_MS : void 0;
			...
			(0, react.useEffect)(() => {
				if (!open && props.mode !== "detail" || auth.status !== "signing-in" || authChallenge?.attemptId === void 0) return;
			...
			const discard = () => {
				...
				if (settings !== void 0) setCapabilities(capabilityOf(settings));
				setFailure(void 0);
				setNotice(void 0);
			};
			const save = async () => {
				if (draft === void 0 || settings === void 0 || capabilities === void 0 || invalid) return;
				if (sourceRevision === void 0) {
			...
			const headerModels = modelCount === void 0 ? "" : t("summaryModels").replace("{count}", String(modelCount));
			...
			const quotaUnsupported = auth.status === "signed-in" && usageUpdatedAt !== void 0 && liveQuota === null;
			const quotaWithheld = auth.status === "signed-out" || auth.status === "reauth-required" || refreshError !== void 0 || quotaUnsupported;
			const quotaProps = providerQuotaHeaderProps(useProviderQuotaCache(CODEX_SETTINGS_NAMESPACE, USAGE_PROVIDER_NAME, liveQuota, {
				answered: authAnswered,
				signedOut: auth.status === "signed-out" || auth.status === "reauth-required",
				withheld: quotaWithheld
			}), {
				dashLabel: t("usage"),
				settled: auth.status === "signed-in" && (refreshError !== void 0 || usageUpdatedAt !== void 0)
			});
			...
			const authChallengeBlock = /* @__PURE__ */ (0, react_jsx_runtime.jsx)(react_jsx_runtime.Fragment, { children: authChallenge !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				style: {
					display: "flex",
					flexDirection: "column",
					gap: 8
			...
			const capabilitiesSection = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
								...
								onChange: (event) => {
									setCapabilities({
										...capabilities,
										enableSearch: event.target.checked
									});
									setNotice(void 0);
								}
							}), t("enableSearch")]
						...
						capabilities.enableSearch ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							...
							onChange: (event) => {
								setCapabilities({
									...capabilities,
									enableImageTool: event.target.checked
								});
								setNotice(void 0);
							}
						}), t("enableImageTool")]
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						capabilities.enableImageGeneration ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "c-extra-grid",
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
								className: "c-field",
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "c-field-label",
									children: t("imageGenerationModel")
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("select", {
									className: "c-input",
			...
			const modelExtra = (item, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "c-extra-grid",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
						className: "c-field",
			...
			if (props.mode === "detail" && SharedDetail !== void 0 && detailCopy !== void 0) {
				const accountActions = auth.status === "signed-in" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					type: "button",
					style: buttonStyle,
					disabled: authBusy,
					onClick: () => {
				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SharedDetail, {
					...
					account: {
						...
						...authChallenge === void 0 ? {} : { body: authChallengeBlock }
					},
					quota: {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
								...
								children: [
									...
									authChallenge === void 0 ? null : authChallengeBlock,
		...
		*/
		...
		function apply(ctx) {
			...
			const settingsForm = ctx.configForms.get(CODEX_SETTINGS_ENTRY_ID);
			const callCodex = (endpoint, payload, signal) => rpc.call("/api", CODEX_RPC_ENDPOINT, {
				endpoint,
				payload
			}, signal);
			...
			const publishAccount = (state) => {
				...
				account.state = state;
				try {
					ctx.get("providerDirectory")?.update(CODEX_SETTINGS_NAMESPACE);
				} catch {}
			};
			...
			const invalidateUsageCache = () => {
				dropPersistedUsageKeys$1([CODEX_SETTINGS_NAMESPACE]);
				try {
					ctx.get("providerDirectory")?.invalidateUsage(CODEX_SETTINGS_NAMESPACE);
				} catch {}
			};
			...
			const startAuth = async () => {
				...
				const result = await callCodex(CODEX_AUTH_BEGIN_ENDPOINT, { method: isLoopback ? "browser" : "device_code" });
				if (!result.ok) throw new Error(result.error.message);
			...
			const fetchModels = async () => {
				...
				return (result.ok ? decodeCodexModelCatalog(result.value) : void 0) ?? officialPickerCatalog();
			};
			...
			ctx.effect(() => {
				...
				readAuthStatus(ac.signal).catch(() => {});
				return () => {
					ac.abort();
					closed = true;
				};
