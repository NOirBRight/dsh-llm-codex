		...
		function CodexPluginCard(props) {
			...
			const refreshAuth = (0, react.useCallback)(async (signal, spin = false) => {
				...
				try {
					...
					if (!liveAuthAttempt(attempt) || signal?.aborted === true) return;
					setAuth(next);
					...
					if (next.status === "signed-in") {
						if (next.quotaError === void 0) {
							...
							rememberHeadlineQuota("llm-codex", "Codex", headerQuotaOf({
								status: "signed-in",
								usage: next.usage
							}, next.usage, t));
			...
			const headerQuota = (auth.status === "signed-out" || refreshError !== void 0 || quotaUnsupported ? void 0 : liveQuota ?? headerQuotaFromCache(peekCachedUsage("llm-codex"))) ?? null;
