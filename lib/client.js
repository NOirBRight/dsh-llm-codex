	...
	factory: (require) => {
		...
		*/
		...
		function decodeCodexAuthStatus$1(value) {
			...
			const credits = recordUsageValue$1(usage.credits);
			if (usage.credits !== void 0 && (credits === void 0 || typeof credits.unlimited !== "boolean" || credits.balance !== void 0 && !nonEmptyString$1(credits.balance))) return {
				status: "error",
				message: "malformed usage response"
			};
			...
			const individual = recordUsageValue$1(usage.individualLimit);
			...
			if (individual !== void 0) {
				...
				if (!nonNegativeNumber$1(remainingPercent) || remainingPercent > 100 || !nonEmptyString$1(remainingText)) return {
					status: "error",
					message: "malformed usage response"
				};
				if (credits === void 0) windows.push({
		...
		*/
		...
		const SIGN_IN_USAGE_DELAY_MS = 1500;
		const cardStyle = {
			overflow: "hidden",
		...
		function CodexPluginCard(props) {
			...
			*/
			const settleUsageAfterSignIn = (0, react.useCallback)(async (isStopped) => {
				for (let attempt = 0; attempt < SIGN_IN_USAGE_ATTEMPTS; attempt += 1) {
					await new Promise((resolve) => {
						window.setTimeout(resolve, SIGN_IN_USAGE_DELAY_MS);
					});
					if (!mounted.current || isStopped()) return;
					const next = await readAuthStatus();
					if (!mounted.current || isStopped()) return;
					applyAuthStatus(next);
					if (next.status === "signed-in" && next.quotaError === void 0 && next.usage !== void 0) return;
				}
