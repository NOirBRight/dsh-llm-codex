...
var CodexAdapter = class extends LlmAdapter {
	...
	*/
	...
	async *stream(options) {
		for await (const chunk of this.current().stream(narrowCodexEscalationSchemas(options))) yield classifyCodexTransientError(chunk);
	}
	...
	async prepareCall(provider, model, signal) {
		...
		return {
			...
			stream: async function* (options) {
				for await (const chunk of inner.stream(narrowCodexEscalationSchemas(options))) yield classifyCodexTransientError(chunk);
			}
		};
	}
};
...
const EVENTS = [CODEX_SEARCH_MODEL_REQUEST_EVENT, CODEX_CONNECT_SEARCH_MODEL_REQUEST_EVENT];
...
*/
var CodexSearchAlpha1Adapter = class {
	context;
	localVocabulary;
	argv;
	hostSessionModule;
	...
	*/
	constructor(options = {}) {
		...
		this.localVocabulary = options.localVocabulary === void 0 ? KNOWN_SESSION_EVENT_TYPES : options.localVocabulary;
	...
	*/
	...
	async installHostVocabulary() {
		if (this.hostSessionModule !== void 0) return this.installHostModule(this.hostSessionModule);
		const href = this.hostSessionHref();
		if (href === void 0) return this.fail("host", "Host session module is not resolvable (missing process.argv[1])");
		try {
			const host = await (this.loadHostSession?.(href) ?? import(__rewriteRelativeImportExtension(href)));
			return this.installHostModule(host);
		} catch (error) {
			return this.fail("host", "Host session module resolution failed: " + errorMessage(error));
		}
	}
	installHostModule(value) {
		if (!isRecord(value)) return this.fail("host", "Host session module is not an object");
		return this.installVocabulary(value["KNOWN_SESSION_EVENT_TYPES"], "host", value["SESSION_FORMAT_VERSION"]);
	}
	hostSessionHref() {
		const entry = this.argv[1];
		if (entry === void 0 || entry.length === 0) return void 0;
		try {
	...
	installVocabulary(value, scope, version) {
		if (version !== ALPHA1_SESSION_FORMAT_VERSION) return this.fail(scope, scope + " session format is not the official alpha.1 version");
		if (!(value instanceof Set)) return this.fail(scope, scope + " session event vocabulary is not a mutable Set");
		if (Object.isFrozen(value) || !Object.isExtensible(value)) return this.fail(scope, scope + " session event vocabulary is frozen or readonly");
		try {
	...
	fail(scope, reason) {
		if (!this.logged.has(scope)) {
			...
			if (this.logSink !== void 0) this.logSink(message);
			else if (this.context !== void 0) this.context.logger.warn(message);
...
function installCodexModelSwitchAdapters(ctx, credentials, settings, options = {}) {
	let imageContext;
	...
	const adapters = {
		...
		search: {
			provider: "codex",
			supportsModel: (model) => (options.searchAvailable?.() ?? true) && settings()?.models.some((candidate) => candidate.id === model && candidate.tools !== false) === true,
			async search(model, request, signal) {
				...
				return new CodexSearchProvider({
					...
					recordRequest: (value) => {
						recordCodexSearchRequest(ctx, value);
					}
...
function createCodexManagementRpcHandler(ctx, auth) {
	return async (endpoint, payload) => {
		if (endpoint === "settings/read") return readConfiguration(ctx);
		if (endpoint === "settings/save") return saveConfiguration(ctx, payload);
		if (endpoint === "auth/status") return {
			ok: true,
			value: await auth.status()
		};
...
function apply(ctx, config) {
	...
	installCodexModelSwitchAdapters(ctx, credentials, resolvedSettings, { searchAvailable: () => searchAlpha1Available });
	const reconcileSearch = async () => {
		if (stopped) return;
		const resolved = resolvedSettings();
		if (resolved === void 0) return;
		const nextRegistration = searchAlpha1Available && current().registerLegacyTools !== false && resolved.enableSearch ? {
