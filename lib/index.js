...
function decodeCodexCatalogModel(value) {
	if (!isRecord$4(value) || typeof value["id"] !== "string" || value["id"].trim().length === 0) return void 0;
	...
	async prepareCall(provider, model, signal) {
		...
		return {
			...
			stream: async function* (options) {
				for await (const chunk of inner.stream(options)) yield classifyCodexTransientError(chunk);
			}
		};
	}
};
...
function apply(ctx, config) {
	...
	const reconcileGenerateImage = async () => {
		if (stopped) return;
		const enabled = resolvedSettings()?.enableImageGeneration === true;
