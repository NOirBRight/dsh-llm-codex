...
function decodeCodexCatalogModel(value) {
	if (!isRecord$3(value) || typeof value["id"] !== "string" || value["id"].trim().length === 0) return void 0;
...
function classifyCodexTransientError(chunk) {
	...
	const code = /^WebSocket (?:error|stream closed before response\.completed)$/iu.test(message) || closed !== null && closed[1] !== "1009" ? "TRANSPORT" : /overloaded|service unavailable/iu.test(message) ? "SERVER" : void 0;
...
var CodexAdapter = class extends LlmAdapter {
	...
	async *stream(options) {
		for await (const chunk of this.current().stream(options)) yield classifyCodexTransientError(chunk);
	}
};
//#endregion
...
//#endregion
//#region lib/types/open-browser.js
...
async function openSystemBrowser(url) {
	...
	if (parsed.protocol !== "https:" && parsed.protocol !== "http:") throw new Error("refusing to open a non-http url");
	if (parsed.username !== "" || parsed.password !== "") throw new Error("refusing to open a url with credentials");
	const launchers = process.platform === "darwin" ? [{
		...
		command: "sensible-open",
		args: [url]
	}];
	let last;
	for (const launcher of launchers) try {
...
function parseWindow(value, now) {
	...
	if (!isRecord$1(value)) throw new Error("Codex returned a malformed rate-limit window");
...
function parseLimit(id, name, value, now) {
	if (value === void 0 || value === null) return void 0;
	if (!isRecord$1(value)) throw new Error("Codex returned malformed rate-limit details");
...
function parseCredits(value) {
	if (value === void 0 || value === null) return void 0;
	if (!isRecord$1(value) || typeof value["has_credits"] !== "boolean" || typeof value["unlimited"] !== "boolean") throw new Error("Codex returned malformed credit details");
...
function parseIndividualLimit(value) {
	if (value === void 0 || value === null) return void 0;
	if (!isRecord$1(value)) throw new Error("Codex returned malformed spend-control details");
	const individual = value["individual_limit"];
	if (individual === void 0 || individual === null) return void 0;
	if (!isRecord$1(individual)) throw new Error("Codex returned a malformed individual limit");
...
function parseCodexUsage(value, now = Date.now()) {
	if (!isRecord$1(value)) throw new Error("Codex returned a malformed usage response");
	const limits = [];
	...
	for (const item of additional ?? []) {
		if (!isRecord$1(item)) throw new Error("Codex returned a malformed additional rate limit");
...
var CodexWebAuth = class {
	...
	async readStoredStatus() {
		...
		try {
			return {
				status: "signed-in",
				usage: await readCodexRateLimits(this.store)
			};
		} catch (error) {
			if (isCodexReauthRequiredError(error)) return {
...
function registerCodexAuthRoutes(ctx, store) {
...
//#endregion
//#region lib/types/search-event.js
...
const CODEX_SEARCH_MODEL_REQUEST_EVENT = "web/codex-search-llm-request";
/** Event type written by `dsh-codex-connect`; registered so those logs still load. */
const CODEX_CONNECT_SEARCH_MODEL_REQUEST_EVENT = "web/openai-codex-search-llm-request";
function addKnownEvents(types) {
	types.add(CODEX_SEARCH_MODEL_REQUEST_EVENT);
	types.add(CODEX_CONNECT_SEARCH_MODEL_REQUEST_EVENT);
}
function hostSessionHref() {
...
function installCodexSearchEvent() {
	if (!(KNOWN_SESSION_EVENT_TYPES instanceof Set)) throw new Error("dsh-llm-codex: this Harness build does not expose an extensible session event vocabulary");
...
async function installHostCodexSearchEvents() {
	const href = hostSessionHref();
	if (href === void 0) return;
	const host = await import(__rewriteRelativeImportExtension(href));
	if (!(host.KNOWN_SESSION_EVENT_TYPES instanceof Set)) return;
	addKnownEvents(host.KNOWN_SESSION_EVENT_TYPES);
}
function recordCodexSearchRequest(ctx, request) {
...
function mapCodexSearchResponse(value) {
	if (!isRecord(value) || typeof value["output"] !== "string") throw new WebError("Codex returned a search response without string output", "WEB_PROVIDER_ERROR");
...
function apply(ctx, config) {
	...
	ctx.inject(["connection"], (connectionCtx) => {
		connectionCtx.connection.rpc.handle(CODEX_RPC_CHANNEL, createCodexRpcHandler(ctx), { authority: "loopback" });
	});
	let stopped = false;
	let searchFiber;
	...
	const reconcileSearch = async () => {
		...
		const fiber = ctx.inject(["web"], (webCtx) => webCtx.web.registerSearchProvider(new CodexSearchProvider({
			...
			recordRequest: (request) => {
				recordCodexSearchRequest(webCtx, request);
			}
		})));
		searchFiber = fiber;
	...
	const reconcileImageTool = async () => {
		if (stopped) return;
		const enabled = resolvedSettings()?.enableImageTool === true;
...
//#endregion
export { CODEX_AUTH_FILENAME, CODEX_AUTH_LOGIN_PATH, CODEX_AUTH_LOGOUT_PATH, CODEX_AUTH_STATUS_PATH, CODEX_BASE_URL, CODEX_CATALOG, CODEX_CHAT_BASE_URL, CODEX_DEFAULT_STREAM_IDLE_TIMEOUT_MS, CODEX_FAST_SERVICE_TIER, CODEX_FAST_SUFFIX, CODEX_LARGE_CONTEXT_SUFFIX, CODEX_LARGE_CONTEXT_WINDOW, CODEX_OFFICIAL_MODELS, CODEX_PROVIDER, CODEX_RPC_CHANNEL, CODEX_SAVE_ENDPOINT, CODEX_SEARCH_PROVIDER, CODEX_SEARCH_URL, CODEX_SETTINGS_NAMESPACE, CODEX_USAGE_URL, CodexAdapter, CodexCredentialStore, CodexReauthRequiredError, CodexSearchProvider, CodexWebAuth, Config, DEFAULT_CODEX_SEARCH_CONTEXT_SIZE, DEFAULT_CODEX_SEARCH_MAX_OUTPUT_TOKENS, DEFAULT_CODEX_SEARCH_MODE, DEFAULT_CODEX_SEARCH_MODEL, DEFAULT_CODEX_SETTINGS, OPENAI_CODEX_PROVIDER, VIEW_IMAGE_TOOL_NAME, apply, applyCodexCatalogWire, applyCodexWirePayload, codexAuthPath, codexAuthStatus, codexResponsesApi, createCodexPiAiProfile, createCodexRpcHandler, decodeCodexCatalogModel, decodeCodexSaveRequest, decodeCodexSaveResult, decodeCodexSettings, defaultDisplayedCatalog, externalWebAccess, hydrateCatalogModel, inject, isCodexReauthRequiredError, loginCodex, logoutCodex, mapCodexSearchResponse, name, officialPickerCatalog, parseCodexUsage, readCodexRateLimits, registerCodexAuthRoutes, resolveAdapterOptions, resolveCodexAccessToken, resolveWireModel, trustedRequest };
