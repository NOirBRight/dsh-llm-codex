...
function officialModelFor(id) {
	const wireId = id.endsWith("-fast") ? id.slice(0, -5) : id;
...
function isFastCatalogId(id) {
	if (!id.endsWith("-fast")) return false;
...
*/
function applyCodexDefaultReasoningMetadata(info, model) {
...
var CodexAdapter = class extends LlmAdapter {
	...
	async resolveModel(provider, model, signal) {
		return applyCodexDefaultReasoningMetadata(await this.current().resolveModel(provider, model, signal), model);
	}
	stream(options) {
