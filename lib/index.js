...
const CODEX_EFFORT_ORDER = CODEX_REASONING_EFFORTS;
...
function effortsForCodexModel(model) {
	...
	if (model.thinking === true) {
		const advertised = model.efforts === void 0 ? void 0 : new Set(model.efforts);
...
//#endregion
//#region lib/types/untrusted-data.js
...
function nonEmptyStringValues(value) {
...
const CODEX_SETTINGS_ENTRY_ID = "llm-codex";
...
const CODEX_RPC_ENDPOINT = "plugin-rpc/codex";
...
function decodeCodexCatalogModel(value) {
	if (!isRecord$5(value) || typeof value["id"] !== "string" || value["id"].trim().length === 0) return void 0;
	...
	if (defaultEffort !== void 0 && isCodexReasoningEffort(defaultEffort)) model.defaultEffort = defaultEffort;
	if (efforts !== void 0 && efforts.length > 0) model.efforts = efforts;
...
function decodeCodexSettings(value) {
	...
	const models = value["models"] === void 0 ? [...DEFAULT_CODEX_SETTINGS.models] : decodeCodexModelCatalog(value["models"]);
...
function thinkingLevelMap(model) {
	...
	const official = officialModelFor(model.id);
	return Object.fromEntries(CODEX_TRANSPORT_LEVELS.map((level) => [level, level !== "off" && efforts.has(level) ? official?.thinkingLevelMap[level] ?? level : null]));
}
...
*/
var CodexCredentialUnusableError = class extends Error {
	constructor(message) {
		super(message);
...
async function assertOwnerOnly(filename) {
	...
	if ((mode & 63) !== 0) throw new CodexCredentialUnusableError(`codex: ${filename} is readable beyond its owner (mode ${(mode & 511).toString(8)}); run "chmod 600 ${filename}" before starting again`);
}
...
function parseDocument(text, filename) {
	...
	if (typeof value !== "object" || value === null || Array.isArray(value)) throw new CodexCredentialUnusableError(`codex: ${filename} must contain an object`);
	const document = value;
	if (document["version"] !== AUTH_FORMAT_VERSION) throw new CodexCredentialUnusableError(`codex: ${filename} has unsupported auth format version ${String(document["version"])}`);
	if (Object.keys(document).some((key) => key !== "version" && key !== "credential")) throw new CodexCredentialUnusableError(`codex: ${filename} contains an unknown top-level field`);
	const raw = document["credential"];
	if (typeof raw !== "object" || raw === null || Array.isArray(raw)) throw new CodexCredentialUnusableError(`codex: ${filename} credential must be an object`);
	...
	if (Object.keys(credential).some((key) => ![
		...
		"expires",
		"accountId"
	].includes(key))) throw new CodexCredentialUnusableError(`codex: ${filename} credential contains an unknown field`);
	if (credential["type"] !== "oauth") throw new CodexCredentialUnusableError(`codex: ${filename} credential type must be oauth`);
	for (const key of [
		"access",
		"refresh",
		"accountId"
	]) if (typeof credential[key] !== "string" || credential[key].length === 0) throw new CodexCredentialUnusableError(`codex: ${filename} credential ${key} must be a non-empty string`);
	if (typeof credential["expires"] !== "number" || !Number.isFinite(credential["expires"]) || credential["expires"] <= 0) throw new CodexCredentialUnusableError(`codex: ${filename} credential expires must be a positive finite number`);
	return {
...
*/
function applyCodexDefaultReasoningMetadata(info, model, override, advertisedEfforts) {
	if (info.reasoning === void 0) return info;
	const reasoning = advertisedEfforts === void 0 ? info.reasoning : {
		...info.reasoning,
	...
	if (!reasoning.efforts.some((effort) => effort.id === defaultEffort)) return reasoning === info.reasoning ? info : {
		...info,
		reasoning
	};
	return {
...
function codexReasoningEfforts(model) {
	if (model === void 0) return void 0;
	const efforts = effortsForCodexModel(model);
	return efforts.length === 0 ? void 0 : efforts;
}
/**
...
var CodexAdapter = class extends LlmAdapter {
	...
	*/
	...
	async prepareCall(provider, model, signal) {
		...
		return {
			model: applyCodexDefaultReasoningMetadata(inner.model, model, catalog?.defaultEffort, codexReasoningEfforts(catalog)),
...
*/
function isCodexCredentialFailure(error) {
	return isCodexReauthRequiredError(error) || error instanceof LlmError && CREDENTIAL_FAILURE_CODES.has(error.code);
}
/**
...
*/
function isCodexCredentialUnusableError(error) {
	for (let current = error; current instanceof Error; current = current.cause) if (current instanceof CodexCredentialUnusableError) return true;
	return false;
}
...
function parseWindow(value, now) {
	...
	if (!isRecord$2(value)) throw new Error("Codex returned a malformed rate-limit window");
...
function parseLimit(id, name, value, now) {
	if (value === void 0 || value === null) return void 0;
	if (!isRecord$2(value)) throw new Error("Codex returned malformed rate-limit details");
...
function parseCredits(value) {
	if (value === void 0 || value === null) return void 0;
	if (!isRecord$2(value) || typeof value["has_credits"] !== "boolean" || typeof value["unlimited"] !== "boolean") throw new Error("Codex returned malformed credit details");
...
function parseIndividualLimit(value) {
	if (value === void 0 || value === null) return void 0;
	if (!isRecord$2(value)) throw new Error("Codex returned malformed spend-control details");
	const individual = value["individual_limit"];
	if (individual === void 0 || individual === null) return void 0;
	if (!isRecord$2(individual)) throw new Error("Codex returned a malformed individual limit");
...
function parseCodexUsage(value, now = Date.now()) {
	if (!isRecord$2(value)) throw new Error("Codex returned a malformed usage response");
	...
	for (const item of additional ?? []) {
		if (!isRecord$2(item)) throw new Error("Codex returned a malformed additional rate limit");
...
async function readCodexRateLimits(store) {
	...
	try {
		...
		throw new LlmError("Codex: the stored credential cannot be used; sign in again with ChatGPT", INVALID_CREDENTIAL_CODE, { cause: error });
	}
	const access = auth?.auth.apiKey;
	const accountId = credential?.type === "oauth" ? credential.accountId : void 0;
	if (access === void 0 || access.length === 0 || typeof accountId !== "string" || accountId.length === 0) throw new LlmError("Codex is signed out", "MISSING_CREDENTIAL");
...
function mapCodexSearchResponse(value) {
	if (!isRecord$1(value) || typeof value["output"] !== "string") throw new WebError("Codex returned a search response without string output", "WEB_PROVIDER_ERROR");
...
function handleCodexEvent(event, parsed) {
	...
	switch (event["type"]) {
		...
		case "response.failed": {
			...
			const error = response !== void 0 && isRecord(response["error"]) ? response["error"] : void 0;
...
function reasoningEfforts(value) {
	...
	return value.flatMap((item) => {
		if (!isRecord$5(item) || typeof item["effort"] !== "string" || !isCodexReasoningEffort(item["effort"])) return [];
		return [item["effort"]];
	});
}
function remoteRows(value) {
	if (!isRecord$5(value) || !Array.isArray(value["models"])) throw new Error("Codex returned an invalid model catalog");
	return value["models"].flatMap((entry) => {
		if (!isRecord$5(entry) || entry["visibility"] !== "list" || entry["supported_in_api"] !== true || typeof entry["slug"] !== "string" || entry["slug"].length === 0) return [];
		const id = entry["slug"];
		...
		const model = {
			...
			...typeof entry["default_reasoning_level"] === "string" && isCodexReasoningEffort(entry["default_reasoning_level"]) ? { defaultEffort: entry["default_reasoning_level"] } : {},
			vision: modalities.includes("image"),
			tools: true
		};
		return nonEmptyStringValues(entry["additional_speed_tiers"]).includes("fast") || Array.isArray(entry["service_tiers"]) && entry["service_tiers"].some((tier) => isRecord$5(tier) && tier["id"] === "priority") ? [model, {
			...model,
			id: id + "-fast",
			name: name + " Fast",
			fast: true
...
async function readCache(filename) {
	...
	if (!isRecord$5(value) || value["version"] !== CACHE_FORMAT_VERSION || !Array.isArray(value["models"])) return void 0;
...
async function fetchRemoteCatalog(store, request) {
	...
	const response = await request("https://chatgpt.com/backend-api/codex/models?client_version=999.0.0", {
		method: "GET",
		redirect: "error",
		headers: {
			authorization: "Bearer " + access,
...
function isSearchableCodexModel(candidate) {
...
function installCodexModelSwitchAdapters(ctx, credentials, settings) {
	...
	const supportsSearchModel = (model) => settings()?.models.some((candidate) => candidate.id === model && isSearchableCodexModel(candidate)) === true;
	const searchModels = () => (settings()?.models ?? []).filter(isSearchableCodexModel).map((candidate) => ({
		id: candidate.id,
...
function readConfig(config) {
	...
	return {
		...
		...enableSearch === void 0 ? {} : { enableSearch },
		...enableImageTool === void 0 ? {} : { enableImageTool },
		...enableImageGeneration === void 0 ? {} : { enableImageGeneration },
		...searchModel === void 0 ? {} : { searchModel },
		...imageGenerationModel === void 0 ? {} : { imageGenerationModel },
		...searchMode === void 0 ? {} : { searchMode },
		...searchContextSize === void 0 ? {} : { searchContextSize },
		...searchMaxOutputTokens === void 0 ? {} : { searchMaxOutputTokens },
		...retryPolicy === void 0 ? {} : { retryPolicy },
		...registerLegacyTools === void 0 ? {} : { registerLegacyTools }
	};
}
/** Parse the retired on-disk literal without advertising it as a supported effort. */
const configuredEffort = z.transform(z.union([...CODEX_REASONING_EFFORTS, z.const("ultra").hidden()]), (effort) => isCodexReasoningEffort(effort) ? effort : void 0);
const configuredEfforts = z.transform(z.array(configuredEffort), (efforts) => efforts.filter((effort) => effort !== void 0));
...
const Config = z.object({
	...
	searchMode: z.union([
		...
		"indexed",
		"live"
	]).default(DEFAULT_CODEX_SEARCH_MODE).volatile(),
	searchContextSize: z.union([
		"low",
		"medium",
		"high"
	]).default(DEFAULT_CODEX_SEARCH_CONTEXT_SIZE).volatile(),
...
async function saveConfiguration(ctx, payload, currentConfig) {
	...
	try {
		...
		if (before.revision !== request.expectedRevision) return failure("settings_conflict", "Codex settings changed; reload before saving");
		...
		return internalError(error instanceof Error && error.message.length > 0 ? safeMessage(error) : "Codex settings save failed");
	}
}
function createCodexManagementRpcHandler(ctx, auth, fetchModels = async () => CODEX_CATALOG, getConfig = () => ({})) {
	return async (endpoint, payload) => {
		try {
			const request = isRecord$5(payload) ? payload : void 0;
			if (endpoint === "models/fetch") return {
				ok: true,
				value: await fetchModels()
			};
			if (endpoint === "settings/save") return saveConfiguration(ctx, payload, getConfig());
			...
			if (endpoint === "auth/begin") {
				const method = request?.["method"] === "device_code" ? "device_code" : "browser";
				return {
					ok: true,
			...
			if (endpoint === "auth/cancel") {
				const attemptId = typeof request?.["attemptId"] === "string" ? request["attemptId"] : void 0;
...
async function handleCodexFetch(request, handler, operator) {
	if (request.headers.get("content-type")?.split(";", 1)[0]?.trim().toLowerCase() !== "application/json") return Response.json({ error: "Expected application/json" }, { status: 415 });
	let body;
	try {
	...
	if (!envelope.success || envelope.data.method !== "plugin-rpc/codex") return Response.json({ error: "Invalid Codex request" }, { status: 400 });
	const payload = isRecord$5(envelope.data.payload) ? envelope.data.payload : void 0;
	if (payload === void 0 || typeof payload["endpoint"] !== "string") return Response.json({ error: "Invalid Codex request payload" }, { status: 400 });
	try {
		const result = await handler(payload["endpoint"], payload["payload"], request.signal, operator);
		if (!result.ok || result.attachments === void 0 || result.attachments.length === 0) return Response.json({
		...
		const attachments = result.attachments.map((attachment, index) => {
			...
			form.append(part, new Blob([attachment.bytes]));
			return {
...
function apply(ctx, config) {
	...
	ctx.inject(["connection"], (connectionCtx) => {
		const handler = createCodexManagementRpcHandler(ctx, auth, () => refreshCodexModelCatalog(credentials), () => current);
	...
	ctx.inject(["settings"], (settingsCtx) => {
		settingsCtx.effect(() => settingsCtx.settings.configure({ auto: false }, ctx.fiber), "dsh-llm-codex: custom settings page policy");
	});
...
//#endregion
export { CODEX_AUTH_ATTEMPT_STATUS_ENDPOINT, CODEX_AUTH_BEGIN_ENDPOINT, CODEX_AUTH_CANCEL_ENDPOINT, CODEX_AUTH_FILENAME, CODEX_AUTH_LOGOUT_ENDPOINT, CODEX_AUTH_STATUS_ENDPOINT, CODEX_BASE_URL, CODEX_CATALOG, CODEX_CHAT_BASE_URL, CODEX_DEFAULT_STREAM_IDLE_TIMEOUT_MS, CODEX_FAST_SERVICE_TIER, CODEX_FAST_SUFFIX, CODEX_LARGE_CONTEXT_SUFFIX, CODEX_LARGE_CONTEXT_WINDOW, CODEX_MODELS_FETCH_ENDPOINT, CODEX_MODELS_URL, CODEX_MODEL_CACHE_FILENAME, CODEX_OFFICIAL_MODELS, CODEX_PROVIDER, CODEX_RPC_ENDPOINT, CODEX_SAVE_ENDPOINT, CODEX_SEARCH_PROVIDER, CODEX_SEARCH_URL, CODEX_SETTINGS_ENTRY_ID, CODEX_SETTINGS_NAMESPACE, CODEX_USAGE_URL, CodexAdapter, CodexCredentialStore, CodexReauthRequiredError, CodexSearchProvider, CodexWebAuth, Config, DEFAULT_CODEX_IMAGE_GENERATION_MODEL, DEFAULT_CODEX_SEARCH_CONTEXT_SIZE, DEFAULT_CODEX_SEARCH_MAX_OUTPUT_TOKENS, DEFAULT_CODEX_SEARCH_MODE, DEFAULT_CODEX_SEARCH_MODEL, DEFAULT_CODEX_SETTINGS, GENERATE_IMAGE_TOOL_NAME, OPENAI_CODEX_PROVIDER, VIEW_IMAGE_TOOL_NAME, apply, applyCodexCatalogWire, applyCodexWirePayload, codexAuthPath, codexAuthStatus, codexResponsesApi, createCodexManagementRpcHandler, createCodexPiAiProfile, decodeCodexCatalogModel, decodeCodexModelCatalog, decodeCodexSaveRequest, decodeCodexSaveResult, decodeCodexSettings, defaultDisplayedCatalog, externalWebAccess, generateImageTool, hydrateCatalogModel, inject, installCodexModelSwitchAdapters, isCodexReauthRequiredError, loginCodex, logoutCodex, mapCodexSearchResponse, name, officialImageGenerationModels, officialPickerCatalog, parseCodexUsage, readCodexRateLimits, refreshCodexAccessToken, refreshCodexModelCatalog, resolveAdapterOptions, resolveCodexAccessToken, resolveWireModel };
