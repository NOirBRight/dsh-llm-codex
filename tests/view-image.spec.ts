...
beforeEach(async () => {
  workspace = await mkdtemp(join(tmpdir(), 'dsh-llm-codex-view-image-'))
  dshHome = await mkdtemp(join(tmpdir(), 'dsh-llm-codex-view-image-home-'))
})
...
async function setup(models?: Array<{ id: string; name: string; vision: boolean }>): Promise<Context> {
...
async function view(context: Context, source: string, model = 'gpt-5.6-sol') {
  return context.tools.execute({
    ...
    name: Codex.VIEW_IMAGE_TOOL_NAME,
    arguments: { source },
...
describe('view_image', () => {
  it('does not register the tool when enableImageTool is off', async () => {
    const context = new Context()
    ctx = context
    await context.plugin(SystemPrompt)
    await context.plugin(ToolRuntime, { mode: 'native' })
  ...
  it.runIf(process.env['CODEX_LIVE_IMAGE_TEST'] === '1')(
    'fetches and stores a real public PNG through the configured network',
    async () => {
      const context = await setup()

      const result = await view(context, 'https://www.gstatic.com/webp/gallery/1.png')
  ...
  it('refuses a model that does not explicitly declare image input', async () => {
    const context = await setup([{ id: 'text-only', name: 'Text only', vision: false }])
    await writeFile(join(workspace, 'pixel.png'), PNG_1X1)

    const result = await view(context, 'pixel.png', 'text-only')
  ...
  it('presents remote URLs as network fetches and local paths as file reads', async () => {
    const context = await setup()
    const definition = context.tools.get(Codex.VIEW_IMAGE_TOOL_NAME)
